...
usage() {
  cat <<'USAGE'
Usage: tools/submit_regional_analysis_campaign.sh --manifest PATH [regional-plan options...]

Build an immutable regional-analysis manifest, submit the monthly pp-long
array, and submit its small reducer with an afterok dependency. Re-running is
safe: completed monthly tasks verify and reuse their published Parquet shards.
USAGE
}
...
"$REPO_ROOT/tools/run_balfrin.sh" regional-plan "${ARGS[@]}"
