...
writes one categorical `PTYPE` field per member/forecast hour plus
`summary.json` and `monitoring.json`. GRIB2 is the default member output format;
NetCDF can be selected with `--output-format=netcdf` and is required for
...
- `src/precip_type_diag/netcdfio.py`: NetCDF member/product read and write helpers.
- `src/precip_type_diag/probabilities.py`: member NetCDF schema and strict
...
- `tools/run_depl_cycle.sh`: explicit DEPL-style wrapper for fixed-cycle
  NetCDF and probability-product production on Balfrin.
...
- Default member-level parallelism uses 8 workers, with a default forecast-hour
  chunk size of 2.
- For manual Balfrin SLURM runs, submit service-node jobs to the generally open
  `pp-short` partition when the expected runtime fits below one hour. Avoid
  restricted elevated-rights partitions such as `pp-production`, `pp-prodntc`,
  and `pp-dispntc` for development or benchmarking jobs.
...
- Runs also write atomic `RUNNING.json`, `DONE.json`, or `FAILED.json` markers
  in the run directory.
- Transient FDB list/retrieve/decode failures can be retried with the
  `--fdb-retries` options. Deterministic scientific, validation, and strict
  probability completeness failures should remain visible.
