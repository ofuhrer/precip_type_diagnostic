...
def test_write_member_ptype_netcdf_roundtrips(tmp_path: Path) -> None:
    output = tmp_path / "member.ptype.nc"
    write_member_ptype_netcdf(
        output,
...
def test_generate_probability_products_requires_all_member_netcdf_diagnostics(tmp_path: Path) -> None:
    stale_output_dir = tmp_path / "ICON-CH1-EPS" / "20260531" / "1800" / "probabilities"
    stale_output_dir.mkdir(parents=True)
    (stale_output_dir / "stale.nc").write_bytes(b"stale")
...
def test_generate_probability_products_writes_step_netcdf(tmp_path: Path) -> None:
    ...
    for member, ptype, rain_probability in [
        ...
        destination = tmp_path / "ICON-CH1-EPS" / date / time_value / member / "lfff00000000.ptype.nc"
        write_member_diagnostic_netcdf(
            destination,
            ptype=ptype,
    ...
    assert set(summary["timings_s"]) == {"preflight_s", "read_s", "aggregate_s", "write_s", "publish_s", "wall_s"}
    assert summary["timings_s"]["read_s"] >= 0.0
...
def test_generate_probability_products_replaces_existing_step_set(tmp_path: Path) -> None:
    ...
    for step in (0, 1):
        write_member_diagnostic_netcdf(
            member_dir / f"lfff000{step}0000.ptype.nc",
            ptype=np.array([[1]], dtype=np.int32),
            hourly_precip_mm=np.array([[1.0]]),
            probabilities=_probabilities(100.0, shape=(1, 1)),
            attrs={"model": "ICON-CH1-EPS", "date": date, "time": time_value, "member": "000", "step": step},
        )

    first = generate_probability_products(
        output_root=tmp_path,
        model="ICON-CH1-EPS",
        date=date,
        time_value=time_value,
    ...
    second = generate_probability_products(
        output_root=tmp_path,
        model="ICON-CH1-EPS",
        date=date,
        time_value=time_value,
